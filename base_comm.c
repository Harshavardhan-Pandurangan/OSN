...
extern int ioRedirect(char *input);
extern void resetRedirect();
...
void runCommand(char *input, int type){
    ...
    int io_set = ioRedirect(input);
    if(io_set == 0){
        // normal execution
    }else if(io_set == 1){
        fprintf(stderr, "Multiple input files given");
    }else if( io_set == 2){
        fprintf(stderr, "Multiple output files given");
    }else if(io_set == 3){
        fprintf(stderr, "Input file not found");
    }else if(io_set == 4){
        fprintf(stderr, "Input file given is a directory");
    }
